...
| 功能 | 說明 |
|------|------|
| 🎴 **互動式牌桌** | 點選式 52 張撲克牌面板，支援點擊空白處自動隱藏、下拉牌桌(Pull-to-refresh)一鍵重置 |
| 📊 **Monte Carlo 模擬** | 後端高效能模擬引擎，支援數千至數萬次模擬迭代 |
| 🤖 **AI 撲克教練** | 串接 Google Gemini AI，針對牌面給出策略建議 (內建斷線自動降級規則引擎) |
| 📈 **Outs 機率分析** | 即時計算可能牌型的 Outs 數量與成牌機率 |
| 📜 **歷史紀錄** | 自動儲存模擬紀錄至 SQLite，可回顧歷史牌局 |
| 🔐 **Google 登入** | 整合 Firebase Authentication，支援 Google 帳號一鍵登入 |
| 🌙 **深色/淺色主題** | 完美支援深淺色切換，精心調校深色模式卡牌與面板的視覺對比質感 |
| ⚙️ **可調整參數** | 自訂模擬次數、動態切換玩家人數 (2-9人) |
